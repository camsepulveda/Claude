#!/usr/bin/env python3
"""Generate TCT Oncology branded ARC Volume Impact document as Word (.docx)."""
...
# TCT brand colors
TCT_NAVY = RGBColor(0x1A, 0x1A, 0x2E)
TCT_DARK = RGBColor(0x2B, 0x2B, 0x3D)
TCT_BLUE = RGBColor(0x00, 0x5C, 0x99)
TCT_LIGHT_BLUE = RGBColor(0x4A, 0x90, 0xD9)
TCT_ACCENT = RGBColor(0x00, 0x96, 0x88)
TCT_WHITE = RGBColor(0xFF, 0xFF, 0xFF)
TCT_GRAY = RGBColor(0x66, 0x66, 0x66)
TCT_LIGHT_GRAY = RGBColor(0xF5, 0xF5, 0xF5)
TCT_RED = RGBColor(0xC0, 0x39, 0x2B)
TCT_GREEN = RGBColor(0x27, 0xAE, 0x60)
TCT_ORANGE = RGBColor(0xE6, 0x7E, 0x22)
...
def add_styled_table(doc, headers, rows, header_color="005C99", col_widths=None):
    ...
    for r_idx, row_data in enumerate(rows):
        for c_idx, val in enumerate(row_data):
            ...
            is_bold_row = text.startswith("**") or text.startswith("CCCUPR") or text.startswith("Auxilio") or "Total" in text or "On-island" in text or "treated" in text.lower() or "HAM" in text
            if is_bold_row and c_idx == 0:
                run.bold = True
                run.font.color.rgb = TCT_BLUE
            ...
            bg = "F5F5F5" if r_idx % 2 == 1 else "FFFFFF"
            if "Total" in str(row_data[0]) or "capture" in str(row_data[0]).lower() or "treated" in str(row_data[0]).lower() or "access" in str(row_data[0]).lower() or "growth" in str(row_data[0]).lower():
                bg = "E8F4FD"
...
def add_section_header(doc, number, title):
    ...
    run.font.size = Pt(9)
    run.font.color.rgb = TCT_LIGHT_BLUE
    run.font.name = "Calibri"
    run.bold = True
    run = p.add_run(title.upper())
    run.font.size = Pt(14)
    run.font.color.rgb = TCT_NAVY
    run.font.name = "Calibri"
    run.bold = True
    # Add line under
    ...
    pBdr = parse_xml(f'<w:pBdr {nsdecls("w")}><w:bottom w:val="single" w:sz="6" w:space="1" w:color="4A90D9"/></w:pBdr>')
    pPr.append(pBdr)

def add_body(doc, text, bold=False, color=None, size=9, space_after=4):
    p = doc.add_paragraph()
...
p.alignment = WD_ALIGN_PARAGRAPH.LEFT
p.space_after = Pt(2)
run = p.add_run("Transplant & Cellular Therapy")
run.font.size = Pt(11)
...
pBdr = parse_xml(f'<w:pBdr {nsdecls("w")}><w:bottom w:val="single" w:sz="12" w:space="1" w:color="005C99"/></w:pBdr>')
pPr.append(pBdr)
...
add_body(doc, "This document demonstrates:", size=9, space_after=4)
...
add_body(doc, "TCT Oncology Dual-Site Presence", bold=True, color=TCT_BLUE, size=10, space_after=2)
...
add_styled_table(doc,
    ...
    [
        ...
        ["Phase 3", "Year 3+ (2029+)", "+ Allogeneic HCT, gene therapy, FACT", "Full-spectrum cellular therapy"],
    ])

add_body(doc, "")
add_body(doc, "Infrastructure at CCCUPR: 3-4 HEPA-filtered positive-pressure rooms, outpatient transplant clinic, 2-3 apheresis collection chairs, pharmacy biological safety hood, ICU with CRS/neurotoxicity capability.", size=8, color=TCT_GRAY)
add_body(doc, "Staffing: 18-22 FTE at launch (Phase 1), scaling to 26-30 FTE (Phase 2), then 32-37 FTE (Phase 3).", size=8, color=TCT_GRAY)

# ============================================================
# SECTION III - DISEASE BURDEN
# ============================================================
add_section_header(doc, "03", "Puerto Rico Hematologic Malignancy Burden")

add_body(doc, "Annual Disease Incidence", bold=True, color=TCT_BLUE, size=10, space_after=2)

add_styled_table(doc,
    ["Cancer Type", "Incidence (per 100K)", "Est. New Cases/Year", "Notes"],
    [
...
add_body(doc, "Sources: PR Central Cancer Registry; SEER Program; Population: 3,184,835 (US Census 2025)", size=7, color=TCT_GRAY)

add_body(doc, "Treatment-Eligible Patient Funnel", bold=True, color=TCT_BLUE, size=10, space_after=2)

add_styled_table(doc,
    ["Stage", "Patients/Year", "Notes"],
    [
        ["New hematologic malignancies diagnosed", "~1,110", "All subtypes"],
        ["Auto SCT-eligible (myeloma, NHL, HD)", "~150-200", "Standard consolidation or salvage"],
        ["Allo-HSCT eligible (AML, ALL, MDS, high-risk NHL)", "~40-60", "Requires matched donor"],
        ["CAR-T eligible (2nd/3rd+ line)", "~80-120", "After >=1 prior line failure"],
        ...
        ["Eligible but untreated (access gap)", "~70-120", "Cannot afford or access mainland"],
    ])

# ============================================================
# SECTION IV - ARC VOLUMES
# ============================================================
add_section_header(doc, "04", "ARC Services — Projected Collection Volumes")

add_body(doc, "The ARC contract must cover the following services for the BMTCI Program. These volumes represent new activity generated by the program — not redistribution of existing ARC workload.", space_after=6)
...
add_body(doc, "")
add_body(doc, "ARC Contract Scope Requirements", bold=True, color=TCT_BLUE, size=10, space_after=2)
...
add_styled_table(doc,
    ...
    [
        ...
        ["CAR-T lymphocyte apheresis & shipment", "Likely gap", "Amend contract to include leukapheresis"],
        ...
        ["Pricing transparency & pass-through costs", "Review", "Negotiate volume-based pricing"],
        ["Capacity guarantees & turnaround times", "Critical", "SLA must guarantee emergent slots"],
        ["Allogeneic donor collections (Phase 3)", "Not yet needed", "Plan for amendment by Year 2-3"],
    ])

# ============================================================
# SECTION V - HOSPITAL VOLUME IMPACT
# ============================================================
add_section_header(doc, "05", "Hospital Volume Impact — Current State vs. Projected")

add_body(doc, "Baseline: Where Patients Go Today (Pre-BMTCI Program)", bold=True, color=TCT_BLUE, size=10, space_after=2)

add_styled_table(doc,
    ["Hospital / Destination", "Consults/Yr", "Auto SCT/Yr", "Allo-HSCT/Yr", "CAR-T/Yr"],
    [
        ...
        ["Mainland US (MD Anderson, Moffitt, etc.)", "—", "40-60", "20-30", "30-50"],
        ["Access gap (eligible, never treated)", "—", "~50-80", "~12-15", "~50-70"],
        ["Total eligible", "~8,000-10,200", "~110-175", "~40-60", "~80-120"],
    ])

add_body(doc, "On-island treatment rates — Auto SCT: ~18-20%  |  Allo-HSCT: ~20-25%  |  CAR-T: 0%", bold=True, color=TCT_RED, size=9)

# Auto SCT Projected
add_body(doc, "")
add_body(doc, "Autologous SCT — Projected Cases Per Year", bold=True, color=TCT_BLUE, size=10, space_after=2)
...
# Allo HSCT Projected
add_body(doc, "")
add_body(doc, "Allogeneic HSCT — HAM via TCT Oncology (Cases Per Year)", bold=True, color=TCT_BLUE, size=10, space_after=2)

add_body(doc, "TCT's dual-site presence creates a direct referral pipeline from CCCUPR to HAM for allo-HSCT. Patients identified at CCCUPR who require allogeneic transplant are referred to HAM through TCT — the only team in Puerto Rico performing allo-HSCT. This increases HAM allo volume rather than competing with it.", space_after=4)
...
# CAR-T Projected
add_body(doc, "")
add_body(doc, "CAR-T Infusions — Projected Cases Per Year", bold=True, color=TCT_BLUE, size=10, space_after=2)
...
# Consults
add_body(doc, "")
add_body(doc, "General Hematology/Oncology Consults Per Year", bold=True, color=TCT_BLUE, size=10, space_after=2)
...
add_body(doc, "Both CCCUPR and HAM consult volumes increase. CCCUPR gains transplant/CAR-T evaluations. HAM gains allo-HSCT evaluations referred from CCCUPR through TCT. The dual-site model is additive, not competitive.", bold=True, size=8, color=TCT_ACCENT)

# ============================================================
# SECTION VI - IMPACT SUMMARY
# ============================================================
...
add_styled_table(doc,
    ...
    [
        ["CCCUPR", "+10-50/yr (new)", "0 to 0-8/yr", "+4-45/yr (new)", "+300-1,100/yr", "Strong positive"],
        ["HAM — TCT", "-2 to -10/yr", "+4 to +23/yr", "No change", "+50-200/yr", "Net positive"],
        ["HIMA San Pablo", "Minimal", "No change", "No change", "Stable", "Neutral"],
        ["Other PR", "No change", "No change", "No change", "-50 to -100/yr", "Minimal"],
        ["Mainland US", "-10 to -45/yr", "-6 to -20/yr", "-5 to -34/yr", "—", "Significant reduction"],
        ["Access gap", "-10 to -68 treated", "-4 to -10 treated", "-8 to -48 treated", "—", "Major positive"],
    ])

add_body(doc, "")
add_body(doc, "Auxilio Mutuo (HAM) — Detailed Impact via TCT Oncology", bold=True, color=TCT_BLUE, size=10, space_after=2)
add_body(doc, "TCT's dual-site model makes HAM a net beneficiary of the BMTCI Program at CCCUPR:")
...
add_body(doc, "The allo-HSCT growth alone more than compensates for any auto SCT volume shift. HAM's transplant program becomes larger and more complex — strengthening its FACT credentials and academic profile.", bold=True, size=9, color=TCT_ACCENT)
...
add_body(doc, "The BMTCI Program at CCCUPR does not cannibalize existing Puerto Rico hospital programs.", bold=True, color=TCT_NAVY, size=11)
...
add_body(doc, "")
add_body(doc, "Transformation of Puerto Rico's Cellular Therapy Landscape", bold=True, color=TCT_BLUE, size=10, space_after=2)
...
add_body(doc, "")
add_body(doc, "For ARC:", bold=True, color=TCT_ACCENT, size=10, space_after=2)
...
add_body(doc, "For HAM:", bold=True, color=TCT_ACCENT, size=10, space_after=2)
...
pPr = p._p.get_or_add_pPr()
pBdr = parse_xml(f'<w:pBdr {nsdecls("w")}><w:top w:val="single" w:sz="6" w:space="4" w:color="005C99"/></w:pBdr>')
pPr.append(pBdr)
